...
class SubscriptionExpiryContainer extends Component {
  ...
  componentDidMount = async () => {
    ...
    for(let i=0; i<branchList.length; i++){
      const { branch_name, id} = branchList[i];
      if(branch_name === defaultBranchName){
        this.setState({currBranchId : id});
      }
    }
    await this.loadDataBasedOnCombo(this.state.currBranchId);
  }
  ...
  downloadSubscriptionData = () => {
   ...
   const { currBranchId } = this.state;
    ...
    for(let i=0; i< branchDetails.length; i++){
      ...
      if(currBranchId === id){
        console.log('currBranchName', currBranchName);
        console.log('currBranchName', branch_name);
  ...
  loadTableData = () => {
    ...
    if (subscriberPhoneNumber) {
      result = subscriptionDetails && subscriptionDetails.filter(dataObj => (dataObj.phoneNumber) === (subscriberPhoneNumber));
      //result = subscriptionDetails && subscriptionDetails.filter(dataObj => (((dataObj.phoneNumber).indexOf(subscriberPhoneNumber)) != -1));
    }
    if (subscriberId) {
      result = subscriptionDetails && subscriptionDetails.filter(dataObj => (dataObj.memberId) === parseInt(subscriberId));
    }
    return result;
  }
